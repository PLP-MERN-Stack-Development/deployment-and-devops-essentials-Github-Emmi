...
const Sidebar = ({ onClose }) => {
  ...
  const handleDirectChatClick = (userId) => {
    if (userId !== user?._id) {
  ...
  return (
    <div className="w-64 sm:w-72 lg:w-64 bg-gray-100 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full">
      {/* Header */}
      <div className="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700">
        <h2 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-white">EmmiDev-Chat</h2>
            ...
            {groupRooms.length === 0 ? (
              ...
              groupRooms.map((room) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 sm:py-2 rounded-lg transition-colors ${
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {room.lastMessage && (
                        <div className="text-xs opacity-75 truncate">
                          Last: {formatRelativeTime(room.updatedAt)}
                        </div>
                      )}
                    </div>
          ...
          <div className="p-3 sm:p-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-1">
              {directMessages.map((dm) => (
                <button
                  key={dm._id}
                  onClick={() => handleRoomClick(dm._id)}
                  className={`w-full text-left px-3 py-2.5 sm:py-2 rounded-lg transition-colors ${
                    currentRoom === dm._id
                      ? 'bg-primary-500 text-white'
                      : 'hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300'
                ...
                >
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate text-sm sm:text-base">{getDirectChatName(dm)}</div>
                      {dm.lastMessage && (
                        <div className="text-xs opacity-75 truncate">
                          Last: {formatRelativeTime(dm.updatedAt)}
                        </div>
                      )}
                    </div>
